...
(function (global) {
  ...
  /**
   * Extract citation numbers and image placeholders from model text.
   */
  ...
  /**
   * Build a simple academic-style references section from source metadata + markers.
   * @param {Array} markers.citations
   * @param {Object} sourceMap  map of sourceId -> { title, pathOrUrl, pages? }
   * @param {Object} [blockMeta] optional { sourceId, pageRange }
   */
  function buildCitationList(markers, sourceMap, blockMeta) {
    ...
    if (!list.length && blockMeta.sourceId) {
      // At least one synthetic entry from the block itself
  ...
  /**
   * Create a knowledge block object (see ARCHITECTURE.md).
   */
  ...
  /**
   * Insert a block into a target DOM element as an editable article section.
  ...
  function insertBlockIntoDOM(container, block, opts) {
    ...
    // Preserve simple newlines; caller can upgrade to markdown renderer later.
    ...
    if (opts.appendCitations && block.citations && block.citations.length) {
      var citeList = buildCitationList(
  ...
  /**
   * Append block + references to a markdown string (for file save).
   */
